...
--~ 	local stack,top={},tree -- top always point to current node 'tree' or the top of the stack
	for line in io.lines(path) do
		...
		if tag=="*" then -- process block lines "*.. CAPTION", a block line without CAPTION will end the current level block
			level=level-1+b_level -- compute real level of current block
			top,not_empty=#stack,match(content, "%S")
			if top>b_level and (not not_empty) and level>top then level=top end -- regulize level to current stack level
			...
			if not_empty then 	-- if CAPTION is meaningful, push it to the parent level node and the stack
			...
			else -- if with an empty caption, refresh the node with the top node of the stack
				node=stack[#stack] 
			end
...
local do_key=function(key,value,ref) -- to do something according to element in table'template' with name 'key' 
	...
	local tp=type(e)
	if tp=="string" then
		if type(value)=="string" then value=string.gsub(value,"%%","%%%%") end
...
local error_in_type=function(tp)
	local error_info="[[ ERROR occured while processing type: "..tp.." ]]"
	if debug_mode then
		return error_info
	else
		print(error_info)
...
eval_element=function(key,value)
	...
	if key=="#" then
		...
		return h and ( do_key(h,arg,hooks) or do_key(h,str2args(arg),template) ) or error_in_type(h)
	else
		return do_key(key,value,template) or error_in_type(key)
	end
end
...
org2file=function(filepath,cls,tmplt_dir)
	...
	local name=(string.match(filepath,"^(.*)%..-$")  or filepath).."-"..cls
	local ext=template.EXT or cls
	local path=name.."."..ext
